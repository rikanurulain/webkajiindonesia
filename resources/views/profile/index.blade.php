...
<form action="{{ route('profile.update-photo') }}" method="POST" enctype="multipart/form-data" class="w-full">
    ...
    <button type="submit" class="w-full py-2 text-sm font-medium text-emerald-700 border border-emerald-600 rounded-md hover:bg-emerald-50 transition">
        Update Foto
                ...
                <!-- Box Daftar Trainer (Yang Tadi Kamu Minta Ubah) -->
                @if($user->role !== 'trainer')
                <div class="mt-6 bg-emerald-700 rounded-xl p-6 text-white shadow-md">
                    <!-- Box Status Trainer Dinamis -->
<div class="mt-6 p-6 rounded-xl text-white shadow-md transition-all duration-300
    ...
    @else
        ...
        <h4 class="font-bold mb-2">Ingin jadi Trainer?</h4>
        <p class="text-xs text-emerald-100 mb-4 leading-relaxed">
            Bergabunglah sebagai pengajar profesional di Kaji Indonesia.
        </p>
        <a href="{{ route('profile.daftar-trainer') }}" class="block w-full bg-white text-emerald-700 py-2 rounded-lg font-bold text-sm text-center hover:bg-gray-100 transition shadow-sm">
            Daftar Sekarang
        </a>
    @endif
</div>
            ...
            <!-- Kolom Kanan: Informasi Akun (List Data Kamu Ada Di Sini) -->
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                    ...
                    <form action="{{ route('profile.update') }}" method="POST" class="space-y-5">
                        ...
                        <div>
                            ...
                            <input type="text" value="{{ $user->email }}" disabled class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-gray-500 cursor-not-allowed">
                        </div>
