...
<div class="tf-page">
    <div class="tf-container">
        ...
        {{-- FORM --}}
        <form
            action="{{ route('profile.simpan-mentor') }}"
            method="POST"
...
<div class="tf-card">
    ...
    <div class="tf-field" style="margin-bottom:0">
        ...
        <input type="url" name="sosmed_facebook"
            value="{{ old('sosmed_facebook') }}"
            placeholder="https://facebook.com/username"
            class="tf-input">
    </div>
</div>

{{-- ===== SEKSI: BIDANG SPESIALISASI ===== --}}
<div class="tf-card">
    ...
    <div class="tf-chips" id="preset-chips-mentor">
        ...
        @endphp
        @foreach ($presetsM as $preset)
            <button
                type="button"
                class="tf-chip {{ in_array($preset, $savedSArr) ? 'tf-chip--active' : '' }}"
    ...
    <div class="tf-selected-tags" id="selected-tags-mentor">
        @foreach ($savedSArr as $item)
            @if (!in_array($item, $presetsM) && $item !== '')
                <span class="tf-tag tf-tag--custom">
                    {{ $item }}
                    <button type="button" onclick="removeTagMentor(this, '{{ $item }}')" class="tf-tag__remove">×</button>
                </span>
            @endif
        @endforeach
    </div>
    ...
    <p class="tf-hint" style="margin-top:8px">
        <span id="spesialisasi-count">{{ count($savedSArr) }}</span> bidang dipilih
    </p>

    {{-- Hidden input --}}
    <input type="hidden" name="bidang_spesialisasi" id="bidang-spesialisasi-value" value="{{ $savedSpesialisasi }}">
</div>
