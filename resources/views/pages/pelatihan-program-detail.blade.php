...
<script>
(function () {
    const block = document.getElementById('absensi-block');
    if (!block) return;

    const mulai   = new Date(block.dataset.mulai);
    const selesai = new Date(block.dataset.selesai);
    const el      = document.getElementById('abs-countdown');
    if (!el) return;
    ...
    function tick() {
        const now      = new Date();
        const upcoming = now < mulai;
        const ms       = upcoming ? (mulai - now) : (selesai - now);

        if (ms <= 0) {
            // Status berubah — reload agar tampilan ikut berubah
            location.reload();
            return;
        }
