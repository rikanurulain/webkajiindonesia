...
@section('page-title', $item ? 'Edit Dokumentasi' : 'Tambah Dokumentasi')
...
<div style="display:flex;align-items:center;gap:8px;margin-bottom:20px;font-size:13px;color:var(--text-muted);">
    <a href="{{ route('admin.dokumentasi.index') }}" style="color:var(--accent);font-weight:600;text-decoration:none;">Dokumentasi</a>
    <span>›</span>
    <span>{{ $item ? 'Edit' : 'Tambah Baru' }}</span>
</div>

<form
    id="form-dokumentasi"
    action="{{ $item ? route('admin.dokumentasi.update', $item) : route('admin.dokumentasi.store') }}"
    method="POST"
...
>
        ...
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <input
                    ...
                    class="form-input"
                    value="{{ old('tanggal_kegiatan', isset($item) ? \Carbon\Carbon::parse($item->tanggal_kegiatan)->format('Y-m-d') : '') }}"
                >
                @error('tanggal_kegiatan') <span class="field-error">{{ $message }}</span> @enderror
            </div>
    ...
    <input type="radio" id="kat-foto" name="kategori" value="foto"
        {{ old('kategori', $item->kategori ?? '') === 'foto' ? 'checked' : '' }}>
    <label for="kat-foto">
...
<div class="kategori-pill">
    <input type="radio" id="kat-video" name="kategori" value="video"
        {{ old('kategori', $item->kategori ?? '') === 'video' ? 'checked' : '' }}>
    <label for="kat-video">
        <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" style="width:15px;height:15px;">
        ...
        Video
            ...
            <div class="form-group full">
                ...
                <textarea name="deskripsi" class="form-textarea" placeholder="Ceritakan singkat tentang kegiatan ini...">{{ old('deskripsi', $item->deskripsi ?? '') }}</textarea>
    ...
    <div class="toggle-wrap">
        <label class="toggle">
            <input
                type="checkbox"
                value="1"
                {{ old('is_published', $item ? (int)$item->is_published : 1) ? 'checked' : '' }}
                id="toggle-published"
            >
        ...
        <div>
            <div class="toggle-label" id="toggle-label-text">
                {{ old('is_published', $item ? $item->is_published : true) ? 'Dipublikasikan' : 'Draft' }}
            </div>
...
<div class="form-card">
    ...
    <div id="section-foto">
            ...
            @if($item && $item->thumbnail)
                <div style="margin-top:10px;">
                    ...
                    <div style="border-radius:12px;overflow:hidden;border:1px solid var(--border);max-height:180px;">
                        <img src="{{ asset('storage/' . $item->thumbnail) }}" style="width:100%;height:180px;object-fit:cover;display:block;">
                    </div>
        ...
        <div class="form-group">
            ...
            @if($item && $item->foto && count($item->foto) > 0)
                <div style="margin-bottom:10px;">
                    <div style="font-size:11px;color:var(--text-muted);margin-bottom:6px;font-weight:600;">FOTO SAAT INI ({{ count($item->foto) }} foto)</div>
                    <div class="existing-foto-grid">
                        @foreach($item->foto as $f)
                        <div class="existing-foto-item">
                            <img src="{{ asset('storage/' . $f) }}" alt="foto">
    ...
    <div id="section-video">
        ...
        <div class="form-group" style="margin-bottom:20px;">
            ...
            @if($item && $item->video_file)
                <div style="margin-top:10px;padding:10px 14px;background:var(--surface2);border:1px solid var(--border);border-radius:10px;font-size:12px;">
                    ...
                    <span style="color:var(--text-muted);">{{ basename($item->video_file) }}</span>
        ...
        {{-- Cover Video (otomatis dari video) --}}
        <div class="form-group" style="margin-bottom:20px;">
            ...
            @if($item && $item->cover_video)
                <div style="margin-bottom:12px;">
                    <div style="font-size:11px;color:var(--text-muted);margin-bottom:6px;font-weight:600;">COVER SAAT INI</div>
                    <div style="border-radius:12px;overflow:hidden;border:1px solid var(--border);max-height:180px;">
                        <img src="{{ asset('storage/' . $item->cover_video) }}" style="width:100%;height:180px;object-fit:cover;display:block;">
                    </div>
                </div>
            @endif
            @error('cover_video') <span class="field-error">{{ $message }}</span> @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input type="url" name="youtube_url" class="form-input"
                ...
                value="{{ old('youtube_url', $item->youtube_url ?? '') }}"
...
<input type="hidden" name="is_published" id="is_published_input"
    value="{{ old('is_published', $item ? (int)$item->is_published : 1) }}">

<div class="action-bar">
    <a href="{{ route('admin.dokumentasi.index') }}" class="btn btn-ghost">
        <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" style="width:15px;height:15px;">
