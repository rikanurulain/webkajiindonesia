...
data-program-mulai="{{ $k->program_mulai ? \Carbon\Carbon::parse($k->program_mulai)->format('Y-m-d\TH:i') : '' }}"
data-program-selesai="{{ $k->program_selesai ? \Carbon\Carbon::parse($k->program_selesai)->format('Y-m-d\TH:i') : '' }}"
...
<div class="program-status-wrap"
     id="prog-status-bar-{{ $k->id }}"
     data-mulai="{{ $progMulai ? $progMulai->timestamp : 0 }}"
     data-selesai="{{ $progSelesai ? $progSelesai->timestamp : 0 }}"
     data-status="{{ $statusProg }}">
    ...
    @if($statusProg === 'belum')
        ...
        <span class="program-countdown" id="prog-timer-{{ $k->id }}">--:--:--</span>
        <span style="font-size:11px;color:var(--text-muted);margin-left:4px">
            ({{ $progMulai->translatedFormat('d M Y, H:i') }} WIB)
        </span>

    @elseif($statusProg === 'aktif')
        ...
        @if($progSelesai)
            ...
            <span class="program-countdown aktif" id="prog-timer-{{ $k->id }}">--:--:--</span>
            <span style="font-size:11px;color:var(--text-muted);margin-left:4px">
                (s/d {{ $progSelesai->translatedFormat('d M Y, H:i') }} WIB)
            </span>
        @else
            <span style="font-size:12px;color:var(--text-muted)">
                Dimulai {{ $progMulai->translatedFormat('d M Y, H:i') }} WIB · Tidak ada batas akhir
            </span>
        @endif
    ...
    @else
        ...
        <span style="font-size:11px;color:var(--text-muted)">
            Berakhir {{ $progSelesai->translatedFormat('d M Y, H:i') }} WIB
        </span>
    @endif
