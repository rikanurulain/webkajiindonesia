...
<?php $__env->startSection('title', 'Dokumentasi Kegiatan – KAJI Indonesia'); ?>
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div class="max-w-2xl">
            ...
            <p class="mt-4 text-lg text-white/90">
                Rekam jejak perjalanan KAJI Indonesia dalam mendampingi dan memberdayakan UMKM Indonesia.
            </p>
        </div>

        
<div class="hidden md:flex flex-shrink-0 items-center justify-center">
    <img src="<?php echo e(asset('storage/logo/logo_kaji.png')); ?>"
         alt="Logo KAJI Indonesia"
         class="w-32 md:w-40 object-contain brightness-0 invert">
</div>
...
<section class="sticky top-[65px] z-40 bg-white border-b border-gray-100 shadow-sm">
    <div class="mx-auto max-w-7xl px-4 py-3">
        <form method="GET" action="<?php echo e(route('media')); ?>" class="flex flex-wrap gap-3 items-center">
            ...
            <div class="flex gap-2">
                <?php $__currentLoopData = [''=>'Semua','foto'=>'📷 Foto','video'=>'🎥 Video']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="<?php echo e(route('media', array_merge(request()->except('kategori','page'), $val ? ['kategori'=>$val] : []))); ?>"
                   class="px-4 py-2 rounded-xl text-sm font-semibold border transition
                          <?php echo e(request('kategori') === $val ? 'bg-primary text-white border-primary' : 'bg-white text-gray-600 border-gray-200 hover:border-primary hover:text-primary'); ?>">
                    <?php echo e($label); ?>
            ...
            <button type="submit" class="px-4 py-2.5 bg-primary text-white text-sm font-semibold rounded-xl hover:bg-emerald-800 transition">
                Cari
            </button>
        </form>
    </div>
...
<section class="mx-auto max-w-7xl px-4 py-12">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        <div class="group bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer"
             onclick="openLightbox(<?php echo e($item->id); ?>)">
            ...
            <div class="relative aspect-video overflow-hidden bg-gray-100">
                ...
                <div class="absolute top-3 left-3">
                    ...
                    <span class="inline-flex items-center gap-1 bg-red-500 text-white text-xs font-bold px-2.5 py-1 rounded-full shadow">
                        ▶ Video
                    </span>
                    <?php else: ?>
                    <span class="inline-flex items-center gap-1 bg-primary text-white text-xs font-bold px-2.5 py-1 rounded-full shadow">
                        📷 Foto
                    </span>
                    <?php endif; ?>
                </div>
            ...
            <div class="p-4">
                <div class="flex items-center gap-1.5 text-xs text-gray-400 mb-2">
                    ...
                    <?php echo e($item->tanggal_kegiatan->translatedFormat('d F Y')); ?>
                ...
                <p class="text-xs text-gray-400 mt-1.5 line-clamp-2 leading-relaxed"><?php echo e($item->deskripsi); ?></p>
                <?php endif; ?>
            </div>
        </div>
        ...
        window.__dok[<?php echo e($item->id); ?>] = {
            judul: <?php echo json_encode($item->judul, 15, 512) ?>,
            deskripsi: <?php echo json_encode($item->deskripsi, 15, 512) ?>,
            tanggal: "<?php echo e($item->tanggal_kegiatan->translatedFormat('d F Y')); ?>",
            kategori: "<?php echo e($item->kategori); ?>",
            foto: <?php echo json_encode($item->foto ? array_map(fn($f) => asset('storage/'.$f), $item->foto) : [], 512) ?>,
            youtube_id: <?php echo json_encode($item->youtube_id, 15, 512) ?>,
            thumbnail: <?php echo json_encode($item->thumbnail_url, 15, 512) ?>,
        };
        </script>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
...
function openLightbox(id) {
    ...
    if (d.kategori === 'video' && d.youtube_id) {
        mediaHtml = `
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\Kaji-indo-main\resources\views/media/index.blade.php ENDPATH**/ ?>